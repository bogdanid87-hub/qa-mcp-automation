...
- **Bounded fix loop** — `npm run fix` stops after 5 attempts by default (the right signal that a problem needs human investigation), overridable with `--max-attempts N`. (`npm run generate`'s own retry-after-generation loop is separate and defaults to 2 — see [generate-test.md](docs/generate-test.md#auto-fix-loop).) Cost is tracked and displayed after each attempt. `--budget N` adds an optional spending cap for shared API keys: fix attempts are aborted *before* an over-budget call is sent (pre-flight estimate), while large generation calls (POMs, e2e specs) only warn so a partial result isn't wasted
