...
export async function generateMockTool(args: GenerateMockArgs): Promise<{
  ...
  // Strip markdown fences from fileContent — local LLM sometimes wraps code
  // in ```typescript ... ``` even when the content is embedded inside JSON.
  const cleanContent = parsed.fileContent
