...
export async function generateMockTool(args: GenerateMockArgs): Promise<{
  ...
  if (!apiKey) return errorContent('Error: ANTHROPIC_API_KEY not set.', { category: 'config', tool: 'generate_mock' });
  ...
  try {
    ...
    return errorContent('Failed to parse the generated mock response.', { tool: 'generate_mock', detail: raw });
  }
