...
export async function generateMockTool(args: GenerateMockArgs): Promise<{
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 2048,
    system: systemBlocks,
    messages: [{ role: 'user', content: TASK_PROMPT(args) }],
  });
  ...
  if (parsed.scope === 'fixture') {
    ...
    await writeFile(filePath, parsed.fileContent, 'utf-8');
    lines.push(`**File written:** ${parsed.fileName}`, '');
  } else {
    lines.push('**Inline snippet (paste inside your test body):**', '```typescript', parsed.fileContent, '```', '');
  }
